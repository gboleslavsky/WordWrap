...
import FormatTextForScreen.WordWrap
  ...
  class WordWrapTest extends FlatSpec {
    "Line width (argument to lines function)" should
      """result in a lists of words(separated by whitespace) with
      overall length with spaces no more than line width""" in {
      assert(WordWrap("1 2 3 4 5 6 7 8 9 ").lines(1)
        ===
        List(List("1"), List("2"), List("3"), List("4"), List("5"), List("6"), List("7"), List("8"), List("9")))
      //if
      assert(WordWrap("123 456 789").lines(1) //if line width can not be observed on a line, output line as is
        ===
